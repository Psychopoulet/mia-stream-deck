...
class OrchestratorStreamDeck extends node_pluginsmanager_plugin_1.Orchestrator {
    ...
    _initWorkSpace() {
        const tablesFile = (0, node_path_1.join)(this._externalResourcesDirectory, "tables.json");
        return new Promise((resolve) => {
            return (0, node_fs_1.stat)(tablesFile, (err, res) => {
                return resolve(!err && res.isFile());
            });
        }).then((isFile) => {
            return isFile ? Promise.resolve() : (0, promises_1.mkdir)(this._externalResourcesDirectory, {
                "recursive": true
            }).then(() => {
